document.addEventListener("DOMContentLoaded", () => {
  ...
  // =============================================================================
  // ANIMATIONS: DESKTOP ONLY (min-width: 768px)
  // =============================================================================
  ...
  ScrollTrigger.matchMedia({
    ...
    "(min-width: 768px)": function () {
      ...
      tlComplianceTransition.to(
        ...
        {
          x: () =>
            calculateComplianceCenterValues(leaderIndex, leaderDot).xMove,
          y: () =>
            calculateComplianceCenterValues(leaderIndex, leaderDot).yMove,
          fill: "#142D21",
      ...
      const finalCircleTarget = document.querySelector(
        "#compliance-circle-target"
      );
      ...
      tlComplianceSequence.to(
        ...
        {
          x: () =>
            calculateLogoValues(sourcePaths.length - 1, scalingDot).xMove,
          y: () =>
            calculateLogoValues(sourcePaths.length - 1, scalingDot).yMove,
          fill: () =>
      ...
      // Setup Final Text
      const greentruthText = document.querySelector(
        "#greentruth-connects-text"
      );
    ...
    // --- Mobile: Disable all animations (max-width: 767px) ---
    "(max-width: 767px)": function () {
      // Clear any GSAP properties that might have been set
      if (sourcePaths) {
        gsap.set(sourcePaths, { clearProps: "all" });
      }
      if (sourceGroup) {
        gsap.set(sourceGroup, { clearProps: "all" });
      }

      // Make compliance section content visible on mobile (no animations)
      ...
      // Ensure all compliance content is visible on mobile
      gsap.set(complianceElements, {
        opacity: 1,
        y: 0,
      });
      ...
      ScrollTrigger.getAll().forEach((trigger) => {
        if (trigger.vars && trigger.vars.trigger) {
          ...
          if (
            triggerId === "pinning" ||
            triggerId === "greentruth-pin" ||
            trigger.vars.trigger === "#compliance-section" ||
            trigger.vars.trigger === "#hero-section" ||
            trigger.vars.trigger === "#greentruth-connects-section"
          ) {
            trigger.kill();
          }
        }
  ...
  ScrollTrigger.matchMedia({
    ...
    // --- Mobile: No animations (max-width: 767px) ---
    "(max-width: 767px)": function () {
      const section = document.querySelector("#get-started-section");
      if (!section) return;
      ...
      // Clear all GSAP properties and ensure everything is visible
      gsap.set([steps, logo, footer, contents], {
        clearProps: "all",
        opacity: 1,
        y: 0,
      });

      // Kill any ScrollTriggers related to get-started section
      ScrollTrigger.getAll().forEach((trigger) => {
